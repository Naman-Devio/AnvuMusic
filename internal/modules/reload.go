...
func init() {
	helpTexts["/reload"] = `<i>Refresh panel — reload admin cache and voice chat state.</i>

<u>Usage:</u>
<b>/reload</b> — Refresh all cached data

<b>🔄 What Gets Refreshed:</b>
• Chat admin list
• Voice chat status
...
• Regular users: 5 min cooldown
• Admins: 2 min cooldown

<b>💡 When to Use:</b>
• After promoting/demoting admins
• Voice chat stuck or not responding
• Permission errors
• Bot behaving incorrectly
• Assistant not joining voice chat

<b>⚠️ Notes:</b>
• May reset room state if admin permissions required
• You can also use the inline buttons to control playback
• If issues persist after reload, try /restart (owner only)
`
}
