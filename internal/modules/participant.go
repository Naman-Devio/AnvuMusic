...
func handleChatAction(m *telegram.NewMessage) error {
	...
	case *telegram.MessageActionChatAddUser:
		...
		for _, uid := range action.Users {
			...
			if uid != botID {
				users, err := m.Client.UsersGetUsers([]telegram.InputUser{
					&telegram.InputUserObj{UserID: uid},
				})
				if err == nil && len(users) > 0 {
					if u, ok := users[0].(*telegram.UserObj); ok && !u.Bot {
						handleNewMember(m.Client, chatID, u)
					}
				}
			}
		}
	...
	case *telegram.MessageActionChatDeleteUser:
		...
		users, err := m.Client.UsersGetUsers([]telegram.InputUser{
			&telegram.InputUserObj{UserID: action.UserID},
		})
		if err == nil && len(users) > 0 {
			if u, ok := users[0].(*telegram.UserObj); ok {
				handleLeftMember(m.Client, chatID, u)
			}
		}
	}
