...
func playTracksAndRespond(
	...
	for i, track := range tracks {
		...
		if i == 0 && (!isActive || force) {
			...
			// Step 1: track snatched
			replyMsg, _ = utils.EOR(replyMsg, F(chatID, "play_snatched", locales.Arg{
				"title": title,
			}), opt)
			time.Sleep(900 * time.Millisecond)
			// Step 2: boosting audio
			replyMsg, _ = utils.EOR(replyMsg, F(chatID, "play_downloading_song", locales.Arg{
				"title": title,
			}), opt)
	...
	if !isActive || (force && len(tracks) > 0) {
		...
		if mainTrack.Artwork != "" && shouldShowThumb(chatID) {
			opt.Media = utils.ResolveArtwork(mainTrack.Artwork, mainTrack.ID)
			opt.Spoiler = true
		}
