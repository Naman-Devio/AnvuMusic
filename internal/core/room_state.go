...
func DeleteRoom(chatID int64) bool {
	...
	// Save current track before Stop() nullifies it, so cleanupFile can still
	// remove the audio file from disk after ffmpeg has been killed.
	room.mu.Lock()
	currentTrack := room.track
	room.mu.Unlock()

	// Stop playback first to kill ffmpeg before deleting files
	room.Stop()
	room.cleanupFile()

	// Also clean up the current track's file since Stop() cleared r.track
	if currentTrack != nil {
		roomsMu.RLock()
		used := isTrackUsed(currentTrack.ID, chatID)
		roomsMu.RUnlock()
		if !used {
			findAndRemove(currentTrack)
		}
	}
