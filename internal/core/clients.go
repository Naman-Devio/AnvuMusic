...
// telegramDCAddrs maps DC ID to production IP:port
var telegramDCAddrs = map[int]string{
...
func decodePyrogramSessionString(
	...
	hostname, ok := telegramDCAddrs[dcID]
	if !ok {
	...
	return &telegram.Session{
		Hostname: hostname,
		AppID: int32(
			uint32(packedData[1])<<24 | uint32(packedData[2])<<16 |
				uint32(packedData[3])<<8 | uint32(packedData[4]),
		),
